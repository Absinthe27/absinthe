...
import com.xude.persistence.model.*;
...
public class PostServiceImpl implements PostService {
    ...
    public List<Post> findPostsByUserId(Long id) {
        List<Post> postsByUserId = postMapper.findPostsByUserId(id);
  ...
  @Override
    public Map<String, Object> findPostsOrderByCommontDate() {
        List<Post> posts = this.postMapper.findPostsOrderByCommontDate();
        Map<String, Object> attributes = new HashMap<>();
        attributes.put("posts", posts);
        return attributes;
    }
    ...
    public Map<String, Object> findPostDetailsAndCommentsByPostId(Long postId) {
        ...
        if (authenticatedUser != null) {
            TVisitor visitor = visitorService.findVisitor(post, authenticatedUser);
            if(visitor==null){
                TVisitor visitorNew  = new TVisitor();
                visitorNew.setNum(1);
                visitorNew.setUser(authenticatedUser);
                visitorNew.setPost(post);
                visitorService.insert(visitorNew);
            }else{
                visitor.setNum(visitor.getNum()+ 1);
                visitorService.updateNum(visitor);
