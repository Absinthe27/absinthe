...
import com.xude.service.DashboardService;
...
public class DashboardController {
	...
	@PreAuthorize("hasAuthority('ADMIN')")
	@RequestMapping(value = "/stickPost/{postId}", method = RequestMethod.PUT)
	public ResponseEntity<?> stickPost(@PathVariable Long postId) {
		if (null == postId) {
			throw new BadRequestException("Path variable postId cound not be null.");
		}
		Post byId = this.postService.findById(postId);
		if(byId.getStick()==null){
			byId.setStick(1l);
		}else{
			byId.setStick(null);
		}
		postService.update(byId);
		return ResponseEntity.ok().build();
	}
