$(document).ready(function() {
	...
	$(".editButtonPost").click(function(event){
	...
	$(".categoryButton").click(function(event){
		event.preventDefault();
		var postId = $(this).data('category-id');
		openPostEditModel(postId);
	})
	...
	$(".postStick").click(function(event){
		var postId = $(this).data('post-id');
		stickPost(postId)
	})
	...
	function stickPost(postId) {
		$.ajax({
			url: "stickPost/" + postId,
			type: "PUT",
			error: function(xhr, status) {
				showMessageModal(false, "置顶or取消置顶！", "置顶or取消置顶帖子（ID：" + postId + "）失败。");
			},
			success: function(result) {
				showMessageModal(true, "置顶or取消置顶！", "置顶or取消置顶帖子（ID：" + postId + "）成功。");
			}
		});
	}
